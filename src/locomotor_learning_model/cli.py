...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional split-belt speed difference in m/s. Defaults to the original "
            "1.0 m/s manuscript simulation."
        ),
    )
    parser.add_argument(
        "--output-dir",
...
def main(argv: list[str] | None = None) -> int:
    ...
    results = run_simulation(
        ...
        split_speed_diff_ms=args.split_speed_diff,
    ...
    if results.param_fixed["SplitOrTied"] == "split":
        print(f"Split speed difference: {results.param_fixed['splitSpeedDiffMS']:.3f} m/s")
