...
from learning.simulate_many_steps_asymmetric_control import (
...
def f_objective_asymmetric_nominal(
    ...
    param_controller.Odd.ydot_at_midstance_nominal_beltframe = float(
        p_input_controller_nominal[1]
    )
    param_controller.Odd.pushoff_impulse_magnitude_nominal = float(
        p_input_controller_nominal[2]
    )
    param_controller.Odd.y_at_midstance_nominal_slopeframe = float(
        p_input_controller_nominal[3]
    )
    param_controller.Odd.sumy_at_midstance_nominal_slopeframe = float(
        p_input_controller_nominal[4]
    )

    param_controller.Even.theta_end_nominal = float(p_input_controller_nominal[5])
    param_controller.Even.ydot_at_midstance_nominal_beltframe = float(
        p_input_controller_nominal[6]
    )
    param_controller.Even.pushoff_impulse_magnitude_nominal = float(
        p_input_controller_nominal[7]
    )
    param_controller.Even.y_at_midstance_nominal_slopeframe = float(
        p_input_controller_nominal[8]
    )
    param_controller.Even.sumy_at_midstance_nominal_slopeframe = float(
        p_input_controller_nominal[9]
    )

    # simulate a handful of steps to compute steady-state energy
    param_fixed.num_steps = param_fixed.Learner.num_steps_per_iteration
    (
        state_var0,
        t_store,
        state_store,
    ...
    lambda_energy_vs_periodicity = param_fixed.lambda_energy_vs_periodicity
    f_objective = (
    ...
    f_objective = (
        param_fixed.lambda_energy_vs_symmetry * f_objective
        + (1 - param_fixed.lambda_energy_vs_symmetry)
        * f_symmetry
        * param_fixed.symmetry_multiplier
    )
